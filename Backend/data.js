...
const seedWebhooks = [
  { id: 'wh1', event: 'Ticket Created',  url: 'https://hooks.zapier.com/hooks/catch/1234567/abcdef/', active: true },
  { id: 'wh2', event: 'Ticket Resolved', url: 'https://hooks.slack.com/services/T123/B456/xyz789abc', active: true },
  { id: 'wh3', event: 'SLA Breached',    url: 'https://api.pagerduty.com/webhooks/ojash-desk', active: false },
];

const seedEmailSettings = {
  supportEmail: 'support@ojashdesk.com',
  replyTo: 'noreply@ojashdesk.com',
  smtpHost: '',
  smtpPort: 587,
  useSSL: true,
};
...
let webhooks = (persisted && persisted.webhooks) || seedWebhooks;
let emailSettings = (persisted && persisted.emailSettings) || seedEmailSettings;
...
// hashes always start with "$2" — anything else is plaintext, re-hash it
let migrated = false;
users.forEach(u => {
...
if (migrated) {
  ...
  storage.saveSync({ tickets, customers, users, kbArticles, chats, cannedResponses, tags, slaPolicy, apiKeys, webhooks, emailSettings, nextTicketNum });
}

function persist() {
  storage.save({ tickets, customers, users, kbArticles, chats, cannedResponses, tags, slaPolicy, apiKeys, webhooks, emailSettings, nextTicketNum });
}

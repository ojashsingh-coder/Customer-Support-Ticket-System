...
const OjashApi = {
  ...
  async createWebhook(data) {
    ...
    if (!r.ok) throw new Error(result.error || 'Failed to create webhook');
    return result;
  },

  async updateWebhook(id, data) {
    const r = await fetch(`${API_BASE}/webhooks/${id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(data)
    });
    if (!r.ok) throw new Error('Failed to update webhook');
    return r.json();
  },
  ...
  async getEmailSettings() {
    const r = await fetch(`${API_BASE}/email-settings`);
    if (!r.ok) throw new Error('Failed to fetch email settings');
    return r.json();
  },
  ...
  async updateEmailSettings(data) {
    ...
    if (!r.ok) throw new Error('Failed to update email settings');
    return r.json();
  },
